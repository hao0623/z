...
basic.forever(function () {
    射擊機率 = randint(1, 2)
    if (射擊機率 == 1) {
        敵人子彈 = game.createSprite(敵人.get(LedSpriteProperty.X), 敵人.get(LedSpriteProperty.Y))
        for (let index = 0; index < 4; index++) {
            敵人子彈.change(LedSpriteProperty.Y, 1)
            basic.pause(100)
        }
        if (敵人子彈.isTouching(自己)) {
            game.removeLife(1)
        }
        敵人子彈.delete()
    }
